...
describe('Auth Module', () => {
    ...
    describe('Registration', () => {
        ...
        describe('Confirm email', () => {
            it('should successfully: /api/v1/auth/email/confirm (POST)', async () => {
                const hash = await (0, supertest_1.default)(mail)
                    .get('/email')
                    .then(({ body }) => body
    ...
    describe('Logged in user', () => {
        ...
        it('should get new refresh token: /api/v1/auth/refresh (POST)', async () => {
            let newUserRefreshToken = await (0, supertest_1.default)(app)
                .post('/api/v1/auth/email/login')
                .send({ email: newUserEmail, password: newUserPassword })
                .then(({ body }) => body.refreshToken);
            newUserRefreshToken = await (0, supertest_1.default)(app)
        ...
        it('should fail on the second attempt to refresh token with the same token: /api/v1/auth/refresh (POST)', async () => {
            const newUserRefreshToken = await (0, supertest_1.default)(app)
                .post('/api/v1/auth/email/login')
                .send({ email: newUserEmail, password: newUserPassword })
                .then(({ body }) => body.refreshToken);
            await (0, supertest_1.default)(app)
        ...
        it('should update profile successfully: /api/v1/auth/me (PATCH)', async () => {
            ...
            const newUserApiToken = await (0, supertest_1.default)(app)
                .post('/api/v1/auth/email/login')
                .send({ email: newUserEmail, password: newUserPassword })
        ...
        it('should update profile email successfully: /api/v1/auth/me (PATCH)', async () => {
            ...
            const hash = await (0, supertest_1.default)(mail)
                ...
                .then(({ body }) => body
                .find((letter) => {
                return (letter.to[0].address.toLowerCase() ===
        ...
        it('should delete profile successfully: /api/v1/auth/me (DELETE)', async () => {
            const newUserApiToken = await (0, supertest_1.default)(app)
                ...
                .send({ email: newUserEmail, password: newUserPassword })
                .then(({ body }) => body.token);
            await (0, supertest_1.default)(app).delete('/api/v1/auth/me').auth(newUserApiToken, {
                type: 'bearer',
            });
            return (0, supertest_1.default)(app)
                .post('/api/v1/auth/email/login')
